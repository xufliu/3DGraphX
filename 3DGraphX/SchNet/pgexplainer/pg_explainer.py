...
class PGExplainer(ExplainerAlgorithm):
    ...
    def train(
        ...
        **kwargs,
        ...
        logits = self.mlp(emb)  #shape:torch.Size([17, 1])
        #inputs = self._get_inputs(emb, edge_index, index)
        #logits = self.mlp(inputs).view(-1)
        node_mask1 = self._concrete_sample(logits, temperature)
        node_mask = node_mask1.sigmoid()
        #edge_weight = edge_mask *edge_weight
        #set_masks(model, edge_mask, edge_index, apply_sigmoid=True) #原始的，好像不行
        ...
        loss = self._loss(y_hat, y, node_mask1)
        loss.backward()
        self.optimizer.step()
    ...
    def _concrete_sample(self, logits: Tensor,
        ...
        #attn_bern = ((logits + random_noise) / temperature).sigmoid()
        #return attn_bern
